  ...
  private BDD calculate_win_FS() {
    ...
    for (iterZ = new FixPoint<BDD>(); iterZ.advance(z);) {
      for (int j = 0; j < sysJustNum; j++) {
        ...
        y = Env.FALSE();
        for (iterY = new FixPoint<BDD>(); iterY.advance(y);) {
          BDD start = sys.justiceAt(j).and(yieldStatesFS(env, sys, z)).or(yieldStatesFS(env, sys, y));
          y = Env.FALSE();
          for (int i = 0; i < envJustNum; i++) {
            BDD negp = env.justiceAt(i).not();
            x = z.id();
  ...
  private BDD calculate_loss() {
    ...
    for (iterZ = new FixPoint<BDD>(); iterZ.advance(z);) {
      for (int j = 0; j < sysJustNum; j++) {
        y = Env.TRUE();
        for (iterY = new FixPoint<BDD>(); iterY.advance(y);) {
          BDD start = ((sys.justiceAt(j).not()).or((env.yieldStates(sys, z.not())).not())).and((env.yieldStates(sys, y.not())).not());
          for (int i = 0; i < envJustNum; i++) {
            x = Env.FALSE();
            c = 0;
            for (iterX = new FixPoint<BDD>(); iterX.advance(x);) {
  ...
  public boolean calculate_strategy(int kind, BDD ini, boolean det) {
    ...
    while (ini_iterator.hasNext()) {
      ...
      while (!st_stack.isEmpty()) {
         ...
         */
        for (BDDIterator all_states = all_succs.iterator(env.moduleUnprimeVars()); all_states.hasNext();) {
        ...
        for (Iterator<BDD> iter_succ = succs.iterator(); iter_succ.hasNext();) {
          BDD primed_cur_succ = Env.prime(iter_succ.next());
          BDD next_op = Env.unprime(sys.trans().and(p_st).and(primed_cur_succ).exist(
          ...
          for (BDDIterator candIter = candidate.iterator(env.moduleUnprimeVars().union(
            ...
            if (det) {
              break; //if we only need one successor, stop here
            }
          }
...
//						}
          ...
          if (rank_j == -1) {
            ...
            if (p_az == 0) {
              input = input.or(p_st.and(primed_cur_succ.and(controlStates(env, sys, (Env.unprime(primed_cur_succ).and((y2_mem[p_j][p_az])))))).and((controlStates(env, sys, Env.FALSE())).not()));
            } else {
              input = input.or(p_st.and(primed_cur_succ.and(controlStates(env, sys, (Env.unprime(primed_cur_succ).and((y2_mem[p_j][p_az])))))).and((controlStates(env, sys, z2_mem[p_az - 1])).not()));
            }
          ...
          if (rank_j != -1 && rank_i != -1 && !(p_st.and(env.justiceAt(rank_i))).isZero()) {
            if (p_az == 0) {
              input = input.or((p_st.and(env.justiceAt(rank_i)).and(primed_cur_succ.and(controlStates(env, sys, (Env.unprime(primed_cur_succ).and((y2_mem[p_j][p_az])))))).and((controlStates(env, sys, (Env.FALSE()))).not())));
            } else {
              input = input.or((p_st.and(env.justiceAt(rank_i)).and(primed_cur_succ.and((controlStates(env, sys, (Env.unprime(primed_cur_succ).and((y2_mem[p_j][p_az]))))))).and((controlStates(env, sys, z2_mem[p_az - 1])).not())));
            }
          }
          if (!input.equals(oldInput) && det) {
          ...
          if (rank_i != -1 && rank_j != -1) {
            if (p_az == 0) {
              if (p_c == 0) {
                input = input.or(((p_st.and((primed_cur_succ.and((controlStates(env, sys, (Env.unprime(primed_cur_succ).and(env.justiceAt(rank_i)).and(x2_mem[p_j][rank_i][p_az][p_c])))))))) //this next clause is probably superfluous because of \rho_1
                        .and((controlStates(env, sys, Env.FALSE())).not())));

              } else {
                input = input.or(((p_st.and(primed_cur_succ.and((controlStates(env, sys, (Env.unprime(primed_cur_succ).and(x2_mem[p_j][rank_i][p_az][p_c - 1]))))))).and((controlStates(env, sys, Env.FALSE())).not())));
              }
            } else {
              if (p_c == 0) {
                input = input.or(((p_st.and(primed_cur_succ.and((controlStates(env, sys, (Env.unprime(primed_cur_succ).and(env.justiceAt(rank_i)).and(x2_mem[p_j][rank_i][p_az][p_c]))))))).and((controlStates(env, sys, z2_mem[p_az - 1])).not())));



              } else {
                input = input.or(((p_st.and(primed_cur_succ.and((controlStates(env, sys, (Env.unprime(primed_cur_succ).and(x2_mem[p_j][rank_i][p_az][p_c - 1]))))))).and((controlStates(env, sys, z2_mem[p_az - 1])).not())));
