...
import wx, wx.richtext, wx.grid, wx.lib.intctrl, wx.lib.masked.numctrl
import sys, os, re
...
def drawParamConfigPane(target, method, proj):
    ...
    for p in method.para:
        ...
        elif p.para_type.lower() in ['float', 'double']:
            param_controls[p] = wx.lib.masked.numctrl.NumCtrl(target, -1, 0.0)
            param_controls[p].SetFractionWidth(3)
            if p.min_val is not None:
                param_controls[p].SetMin(p.min_val)
                param_controls[p].SetLimited(True)
            if p.max_val is not None:
